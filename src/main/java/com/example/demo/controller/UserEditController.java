...
public class UserEditController {
	...
	public String view(Model model, UserEditForm form) throws Exception {
		var userid = (String) session.getAttribute(SessionKeyConst.SELECETED_LOGIN_ID);
		var userInfoOpt = service.searchUserInfo(userid);
	...
	public String updateUser(Model model, UserEditForm form, @AuthenticationPrincipal User user) {
		...
		updateDto.setUserid((String) session.getAttribute(SessionKeyConst.SELECETED_LOGIN_ID));
	 ...
	 */
	private void setupCommonInfo(Model model,  com.example.demo.entity.User userInfo) {
